...
<head>
  ...
  <title>New Hire Developer Request</title>
  <style>
...
<div class="card">
  ...
  <div class="hd">
    ...
    <div class="badge"><span class="badge-dot"></span> Hire Developer Request</div>
    <div class="hd-title">New Request to Hire a Developer</div>
    <div class="hd-sub">Submitted {{ $hire->created_at->format('D, d M Y \a\t H:i') }} UTC</div>
  </div>

  <div class="alert-bar">
    &#x1F4E5; <strong>{{ $hire->full_name }}</strong> wants to hire <strong>{{ $hire->developer_type }}</strong>.
  </div>
  ...
  <div class="body">
    ...
    <div class="grid">
      <div class="cell">
        ...
        <div class="cell-val">{{ $hire->full_name }}</div>
      </div>
      <div class="cell">
        <div class="cell-label">Company</div>
        <div class="cell-val">{{ $hire->company ?: '—' }}</div>
      </div>
      <div class="cell">
        <div class="cell-label">Email</div>
        <div class="cell-val"><a href="mailto:{{ $hire->email }}">{{ $hire->email }}</a></div>
      </div>
      <div class="cell">
        <div class="cell-label">Phone</div>
        <div class="cell-val">{{ $hire->phone ?: '—' }}</div>
      </div>
    </div>

    <div class="sec-label">Hiring Details</div>
    <div class="grid">
      <div class="cell">
        <div class="cell-label">Developer Type</div>
        <div class="cell-val"><span class="tag">{{ $hire->developer_type }}</span></div>
      </div>
      <div class="cell">
        <div class="cell-label">Engagement Type</div>
        <div class="cell-val">{{ $hire->engagement_type ?: 'Not specified' }}</div>
      </div>
      <div class="cell">
        <div class="cell-label">Team Size</div>
        <div class="cell-val">{{ $hire->team_size ?: 'Not specified' }}</div>
      </div>
      <div class="cell">
        <div class="cell-label">Estimated Budget</div>
        <div class="cell-val">{{ $hire->budget ?: 'Not specified' }}</div>
      </div>
    </div>

    <div class="sec-label">Project / Requirement Details</div>
    <div class="msg-box">
      <div class="msg-text">{{ $hire->description }}</div>
    </div>

    <div class="meta">
      <div class="meta-item"><strong>Request ID:</strong> #{{ $hire->id }}</div>
      <div class="meta-item"><strong>Submitted:</strong> {{ $hire->created_at->diffForHumans() }}</div>
      <div class="meta-item"><strong>IP:</strong> {{ $hire->ip_address ?? '—' }}</div>
    </div>

    <div class="cta">
      <a href="mailto:{{ $hire->email }}?subject=Re: Your request to hire {{ $hire->developer_type }} — {{ config('app.name') }}" class="cta-btn">
        &#x21A9; Reply to {{ $hire->full_name }}
      </a>
    </div>
