...
<div class="card">
  ...
  <div class="hd">
    <div class="logo-row">
      ...
      <div><div class="logo-name">Kawach<span>.</span>Tech</div></div>
    </div>
    <div class="badge"><span class="badge-dot"></span> Job Application</div>
    <div class="hd-title">New Application Received</div>
    <div class="hd-sub">Submitted {{ $application->created_at->format('D, d M Y \a\t H:i') }} UTC</div>
  </div>

  <div class="alert-bar">
    &#x1F4E5; <strong>{{ $application->full_name }}</strong> applied for <strong>{{ $application->job_title }}</strong>. Resume is attached to this email.
  </div>
  ...
  <div class="body">
    ...
    <div class="grid">
      <div class="cell">
        ...
        <div class="cell-val">{{ $application->full_name }}</div>
      </div>
      <div class="cell">
        <div class="cell-label">Experience</div>
        <div class="cell-val">{{ $application->experience ?: 'Not specified' }}</div>
      </div>
      <div class="cell">
        <div class="cell-label">Email</div>
        <div class="cell-val"><a href="mailto:{{ $application->email }}">{{ $application->email }}</a></div>
      </div>
      <div class="cell">
        <div class="cell-label">Phone</div>
        <div class="cell-val">{{ $application->phone ?: '—' }}</div>
      </div>
      <div class="cell">
        <div class="cell-label">LinkedIn</div>
        <div class="cell-val">
          @if($application->linkedin_url)
            <a href="{{ $application->linkedin_url }}">{{ $application->linkedin_url }}</a>
          @else
            —
          @endif
        </div>
      ...
      <div class="cell">
        <div class="cell-label">Portfolio</div>
        <div class="cell-val">
          @if($application->portfolio_url)
            <a href="{{ $application->portfolio_url }}">{{ $application->portfolio_url }}</a>
          @else
            —
          @endif
        </div>
    ...
    <div class="grid">
      <div class="cell full">
        <div class="cell-label">Applying For</div>
        <div class="cell-val"><span class="tag">{{ $application->job_title }}</span></div>
      </div>
    </div>

    @if($application->cover_letter)
    <div class="sec-label">Cover Letter</div>
    <div class="msg-box">
      <div class="msg-text">{{ $application->cover_letter }}</div>
    </div>
    @endif

    <div class="meta">
      <div class="meta-item"><strong>Application ID:</strong> #{{ $application->id }}</div>
      <div class="meta-item"><strong>Submitted:</strong> {{ $application->created_at->diffForHumans() }}</div>
      <div class="meta-item"><strong>IP:</strong> {{ $application->ip_address ?? '—' }}</div>
    </div>

    <div class="cta">
      <a href="mailto:{{ $application->email }}?subject=Re: Your application for {{ $application->job_title }} — {{ config('app.name') }}" class="cta-btn">
        &#x21A9; Reply to {{ $application->full_name }}
      </a>
    </div>
